...
echo "=== ForbCheck-Beta Installation Log ===" > "$LOG_FILE"
...
curl -sfL "$REPO_RAW_URL/forb-beta.sh" -o "$INSTALL_DIR/forb-beta.sh" >> "$LOG_FILE" 2>&1
curl -sfL "$REPO_RAW_URL/forb-beta_completion.sh" -o "$INSTALL_DIR/forb-beta_completion.sh" >> "$LOG_FILE" 2>&1

if [ ! -s "$INSTALL_DIR/forb-beta.sh" ]; then
    echo -e "${RED}Error: Could not download forb-beta.sh. Check your internet connection or GitHub repository.${NC}"
    log_action "ERROR: Failed to download forb-beta.sh."
    exit 1
fi
...
configure_shell() {
    ...
    if [ -f "$rc_file" ]; then
        ...
        if [ -s "$INSTALL_DIR/forb-beta_completion.sh" ]; then
            if ! grep -q "forb-beta_completion.sh" "$rc_file"; then
                echo -e "\n# ForbCheck-Beta Autocompletion" >> "$rc_file"
                if [ "$is_zsh" = true ]; then
                    echo "autoload -U +X compinit && compinit" >> "$rc_file"
                    echo "autoload -U +X bashcompinit && bashcompinit" >> "$rc_file"
                fi
                echo "source $INSTALL_DIR/forb-beta_completion.sh" >> "$rc_file"
            fi
        fi
...
log_action "Starting 'forb-beta.sh -gp' to fetch presets..."
if yes | bash "$INSTALL_DIR/forb-beta.sh" -gp >> "$LOG_FILE" 2>&1; then
    log_action "Presets fetched successfully."
else
    echo -e "${RED}[✖] Error while fetching presets!${NC}"
    echo -e "Check $LOG_FILE for details."
    log_action "ERROR: 'forb-beta.sh -gp' failed."
    exit 1
fi
